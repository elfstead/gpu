# Historical milestone plan — through 2026-09-15

Archived when the two-consumer checkpoint was consolidated. This records the
sequence of decisions, not the current work queue. See [the working plan](plan.md).
...
verified (`39b8c16`). [Modern compute, graphics/heaps and GGML now pass on physical RADV hardware](hardware-validation.md);
provisioned remote execution-CI checks remain pending.
...
This completes the selected D1/D3/D4 cleanup. The subsequent D1 milestone,
[GGML FP16 weights with FP32 arithmetic](ggml-mixed-precision.md), is also complete:
