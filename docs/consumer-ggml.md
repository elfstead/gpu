...
| D1 | Retain trusted Vulkan 1.2-targeted SPIR-V: `main`, descriptor-free, core-required features plus buffer device address. Successful compute-device creation guarantees the selected modern Vulkan baseline, not Vulkan 1.2-device compatibility. Probe numeric flags only report support, not enabled optional arithmetic. Consumer metadata defines root layout, local size and dispatch math. Offline validation of checked-in modules is required; runtime header checks are not reflection or validation. No optional executable variants needed. |
