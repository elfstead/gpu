...
def inspect(reflection, assembly):
    ...
    for index, (field, native_id) in enumerate(zip(fields, members[1:])):
        ...
        if typ["kind"] == "pointer":
            pointee = {"uint": ["OpTypeInt", "32", "0"], "float": ["OpTypeFloat", "32"]}.get(typ["valueType"])
            require(pointee is not None and physical, "only enabled uint/float pointers supported")
            require(native[:2] == ["OpTypePointer", "PhysicalStorageBuffer"] and
                    definitions[native[2]] == pointee, "pointer type mismatch")
            require(re.findall(rf"OpDecorate {re.escape(native_id)} ArrayStride (\d+)", assembly) == ["4"],
...
def header(reflection, assembly, binary, source, name="transform"):
    ...
    cursor = 0
    for index, (field_name, typ, offset, width) in enumerate(fields):
        if offset > cursor:
            lines.append(f"    uint8_t padding_{index}[{offset-cursor}];")
        lines.append(f"    {typ} {field_name};")
        cursor = offset + width
    if size > cursor:
        lines.append(f"    uint8_t padding_tail[{size-cursor}];")
    if fields:
        lines += [f"}} {type_name};", f'_Static_assert(sizeof({type_name}) == {size}, "root size");',
                  f'_Static_assert(_Alignof({type_name}) == {alignment}, "root alignment");']
    for field_name, _, offset, _ in fields:
        lines.append(f'_Static_assert(offsetof({type_name}, {field_name}) == {offset}, "{field_name} offset");')
    if local:
...
def compile_source(source, output_dir, compiler, stage="compute"):
    ...
    run(compiler, str(source), "-target", "spirv", "-profile", "spirv_1_5",
        ...
        "-entry", "main", "-stage", stage, "-reflection-json", str(reflection), "-o", str(binary))
