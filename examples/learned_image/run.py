...
def main():
    ...
    for filename in ("hidden.comp", "denoise.comp", "process.comp", "poison.comp", "display.frag", "fullscreen.vert"):
        source = ROOT / "examples/shaders/fullscreen.vert" if filename == "fullscreen.vert" else HERE / filename
        destination = shaders / f"{filename}.spv"
        command(os.getenv("GLSLC", "glslc"), "--target-env=vulkan1.4", str(source), "-o", str(destination))
        command("spirv-val", "--target-env", "vulkan1.4", str(destination))
    ...
    command(*shlex.split(os.getenv("CC", "cc")), "-std=c11", "-O2", "-DNDEBUG",
            "-Wall", "-Wextra", "-Werror", "-I", "include", str(HERE / "app.c"),
            "-L", str(library), f"-Wl,-rpath,{library}", "-logpu", "-o", str(executable))
    if args.check:
        print("Learned-image CPU/build/SPIR-V checks PASS; no GPU execution")
        return
    ...
    for name, cases in groups.items():
        ...
        process = command(str(executable), str(shaders), str(reference / "weights.f32"),
    ...
    summary = (f"Learned-image Vulkan PASS: {len(manifest['cases'])} cases in both normal/diagnostic modes; "
