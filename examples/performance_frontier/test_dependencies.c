...
static unsigned submits, waits, drains, pools, events;
...
static DependencyControl fixture(void) {
    ...
    submits=waits=drains=pools=events=0;
    DependencyControl d={0}; d.strategy="split"; d.base.policy="replay"; d.base.count=2;
    d.event=(VkEvent)(uintptr_t)1; d.destroy_event=event_mock;
...
int main(void) {
    ...
    DependencyControl d=fixture(); d.base.slots[0].pending=1; d.base.slots[0].batch.value=1;
    assert(!dependency_window(&d,1,0,&frame,&wall) && !submits);
    ...
    for (unsigned i=0;i<3;++i) {
        ...
        assert(submits==1 && !waits && drains==1 && !d.base.slots[0].pending);
        dependency_destroy(&d); assert(pools==1 && events==1 && !waits);

        d=fixture(); wait_result=failures[i];
        assert(!dependency_window(&d,1,0,&frame,&wall));
        assert(submits==1 && waits==1 && !d.base.slots[0].pending);
        assert(drains==(failures[i]!=VK_ERROR_DEVICE_LOST));
        dependency_destroy(&d); assert(pools==1 && events==1 && waits==1);
    }
    puts("P4 pending/submit/wait/loss failures drain before command/event release PASS");
    return 0;
}
