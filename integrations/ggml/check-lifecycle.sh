...
target/ggml-integration/ogpu-ggml-lifecycle integrations/ggml/shaders normal 2>&1 | tee "$log"
if rg 'Validation Error:' "$log"; then exit 1; fi
for mode in live-backend live-buffer; do
    status=0
    target/ggml-integration/ogpu-ggml-lifecycle integrations/ggml/shaders "$mode" > "$log" 2>&1 || status=$?
    test "$status" = 134
