#!/usr/bin/env bash
# Optional training experiment. Never replaces the pinned regression fixture.
set -euo pipefail
repo=$(cd -- "$(dirname -- "$0")/../.." && pwd)
source_dir=${1:?usage: bash integrations/ggml/train.sh /path/to/pinned/ggml}
...
if [[ ! -f "$data_dir/mnist-fc-trained.gguf" ]]; then
    ...
    mv -- "$model_tmp" "$data_dir/mnist-fc-trained.gguf"
fi
sha256sum "$data_dir/mnist-fc-trained.gguf"
