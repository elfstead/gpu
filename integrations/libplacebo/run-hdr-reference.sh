#!/usr/bin/env bash
# Native-first gate only: no OGPU execution or tolerance relaxation here.
set -euo pipefail
...
# Exit 2 denotes the declared exact-alpha numerical gate, not an execution pass.
if test "$status" = 2; then
    rg -q '^HDR-reference creates=6 compute=9 raster=9 .* GATE FAILED \(native only\)$' "$capture/run.log" || exit 1
    echo 'Native capture complete; acceptance gate failed. See docs/libplacebo-hdr.md.' >&2
elif test "$status" = 0; then
    rg -q '^HDR-reference creates=6 compute=9 raster=9 .* PASS \(native only\)$' "$capture/run.log" || exit 1
fi
exit "$status"
