...
tests=${OGPU_BACKEND_TESTS:-$out/backend-tests}
log=$(mktemp "$out/backend-checks.XXXXXXXX.log")
ulimit -c 0
for mode in unorm16-sampling specialization-update partial-upload unsupported-image unsupported-shader \
    ...
    "$tests" "$mode" 2>&1 | tee -a "$log"
done
for mode in frame-reuse frame-capacity frame-partial frame-staging frame-destroy \
            frame-pending frame-poll-error frame-submit-error queued-specialization queued-bank-reuse; do
    "$tests" "$mode" batched 2>&1 | tee -a "$log"
done
status=0
"$tests" live-child >> "$log" 2>&1 || status=$?
test "$status" = 134
...
rg -q '^adapter unorm16 sampling/transfer live=0 PASS$' "$log"
