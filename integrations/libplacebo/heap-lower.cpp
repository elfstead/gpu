...
int main(int argc, char **argv) {
    try {
        if (argc == 2 && std::string(argv[1]) == "--test") {
            ...
            std::cout << "heap-lower body preservation, slots and rejection tests PASS\n";
            return 0;
        }
        if (argc != 3)
            throw std::runtime_error("usage: heap-lower input.glsl output.glsl");
        ...
        unsigned count = 0;
        const auto result = lower(source.str(), count);
