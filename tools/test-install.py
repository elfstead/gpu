...
def main():
    ...
    if not args.no_gpu:
        ...
        if args.host_view:
            mapped = environment.copy()
            mapped["OGPU_EXAMPLE_HOST_VIEW"] = "1"
            run(["./transform"], cwd=application, env=mapped)
        if args.split:
            split = environment.copy()
            split["OGPU_EXAMPLE_SPLIT"] = "1"
            run(["./transform"], cwd=application, env=split)
